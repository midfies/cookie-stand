...
console.log(store);
cookieTableJS();
for (var loop = 0;loop < store.length;loop++){
  store[loop].calculaterTossers();
  console.log(store[loop].randomCustPerHour);
  console.log(store[loop].tosserPerHour);
  store[loop].displayAll();
}
